...
DoubleScriptedArray::DoubleScriptedArray(int cols, int rows){
    ...
    if(cols > 0 && rows >= 0){
        ...
        size = ((ROWS > 0) ? (COLS * ROWS) : COLS); // avoid multiply by zero error
    }else{
...
int DoubleScriptedArray::getSize() const{
    return size;
}
...
int& DoubleScriptedArray::operator()(unsigned int subRow, unsigned int subCol) const{
    // check for subscript out-of-range error
    if(subRow < 0 || subRow >= ROWS || subCol < 0 || subCol >= COLS){
        throw std::out_of_range("Subscript out of range");
    }
