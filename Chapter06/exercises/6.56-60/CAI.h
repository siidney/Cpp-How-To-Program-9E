...
class CAI{
    private:
        ...
        template<typename T> bool checkAnswer(T answer, T solution){
    ...
    public:
        CAI() : _questionCount(0), _correct(0), _incorrect(0), _difficulty(0),
                _randomProblem(0)
            {initialise();}
        ~CAI(){_correctResponses.clear();_incorrectResponses.clear();}
