...
void ReservationSystem::printMenu(){
    ...
    if(((choice == 0 || choice == 1) && (_availability == Availability::BOTH)) ||
    ...
    // prevent incorrect values triggering this
        ...
        std::cout << "\n\n ************\n"
                  ...
                  << "\nWould you like a seat in " << ((choice == 0) ? "Second" : "First")
                  << " class? ";
        std::cin >> change;
...
bool ReservationSystem::isSeatAvailable(int seat){
    return !_plane[seat];
}
...
int ReservationSystem::bookSeat(int cl){
    ...
    for(seat; seat<=seatClass; ++seat){
        if(isSeatAvailable(seat)){
            _plane[seat] = !_plane[seat];

            // set availability based upon seatClass limits
            if(seat == 5)
                _availability = Availability::SECOND;

            if(seat == 10)
                _availability = Availability::FIRST;
