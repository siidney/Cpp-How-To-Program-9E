...
// SETTERS
void Rectangle::setRectangle(Coord2D topL, Coord2D botL, Coord2D topR, Coord2D botR){
    if((topL.getX() < 20.0f && topL.getY() < 20.0f) &&
       ...
       (botR.getX() < 20.0f && botR.getY() < 20.0f)){
        ...
        if(topL.getX() != botL.getX() || topR.getX() != botR.getX()){
            throw std::invalid_argument("Left/Right side X coordinates do not match.");
        }else{
            ...
            if(!isRectangle()){
                throw std::invalid_argument("Coordinates do not represent a rectangle");
            }
        }
    }else{
