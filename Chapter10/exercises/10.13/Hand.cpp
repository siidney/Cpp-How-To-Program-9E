...
Hand::Hand(bool dealer, std::string n) : isSorted(false), dealer(dealer), handScored(false){
    if(dealer){
        name = "Dealer";
    }else{
        name = n;
                ...
                if(hand[i].getFace() == hand[j].getFace()){
                    ...
                    for(unsigned int k=0; k<HAND_SIZE; ++k){
                        for(unsigned int l=k+1; l<HAND_SIZE; ++l){
                            if(hand[k].getFace() != pairFace1 && hand[k].getFace() == hand[l].getFace()){
...
void Hand::playerMove(){
    ...
    while(true){
        ...
        if(numDiscard > MAX_DISCARD){
            std::cout << "Can discard a maximum of 3 cards. Try again" << std::endl;
        }else{
            break;
        }
    }
    ...
    for(unsigned int i=0; i<numDiscard; ++i){
        std::cout << "Enter the index (0-4) of card number " << i+1 << " to discard: ";
...
bool Hand::draw(unsigned int n){
    //std::cout << "n: " << n << " cardsRemaning: " << cardsRemaining() << std::endl;
    ...
    if(hand.size() == HAND_SIZE){
        ...
        std::cout << "Cannot draw more than " << MAX_DISCARD <<  " cards. Try again\n";
    }
    return false;
}
// SCORING FUNCTIONS
