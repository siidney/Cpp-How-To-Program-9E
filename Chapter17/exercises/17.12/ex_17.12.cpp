    ...
    while((choice = enterChoice()) != END){
        switch(choice){
            ...
            case NEW: // generate new list
                do{
                    number = getNumber("Enter a 7 digit telephone number (xxx-xxxx)");
...
void generateWords(const std::string& number){
    ...
    for(int i=0; i<4; ++i){
        if(letters[(sanNum[0] - '0') - 2][i] == ' ')
            continue;
        for(int j=0; j<4; ++j){
            if(letters[(sanNum[1] - '0') - 2][j] == ' ')
                continue;
            for(int k=0; k<4; ++k){
                if(letters[(sanNum[2] - '0') - 2][k] == ' ')
                    continue;
                for(int l=0; l<4; ++l){
                    if(letters[(sanNum[3] - '0') - 2][l] == ' ')
                        continue;
                    for(int m=0; m<4; ++m){
                        if(letters[(sanNum[4] - '0') - 2][m] == ' ')
                            continue;
                        for(int n=0; n<4; ++n){
